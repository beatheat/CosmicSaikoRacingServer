...
namespace CSRServer
{
    internal class GameScene : Scene
    {
        ...
        public override void Load()
        {
            ...
            {
                GamePlayer gamePlayer = new GamePlayer(lbPlayer.clientId, playerList.Count, lbPlayer.nickname);
        ...
        private EdenData UseCard(string clientId, EdenData data)
        {
            if (phase == Phase.Preheat)
            {
                int useCardIndex = data.Get<int>();
                GamePlayer player = playerMap[clientId];
                object[]? result = player.UseCard(useCardIndex, out var effectType);
                if (result == null)
                {
        ...
        private EdenData RollResource(string clientId, EdenData data)
        {
            if (phase == Phase.Preheat)
            {
                List<int> resourceFixed = data.Get<List<int>>();
                GamePlayer player = playerMap[clientId];
