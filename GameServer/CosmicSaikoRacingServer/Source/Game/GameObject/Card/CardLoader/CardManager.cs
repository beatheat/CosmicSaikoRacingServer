...
internal static class CardManager
{
	...
	private static readonly Dictionary<char, ResourceType> _symbolToResourceType = new Dictionary<char, ResourceType>
	{
		...
		for (int i = 0; i < jsonArray.Count; i++)
		{
			...
			try
			{
				...
				if (jsonVariable != null)
					variables = ParseCardVariable(jsonVariable.ToString());
	...
	public static Card GetRandomCardWithCondition(int rankMin, int rankMax)
	{
		...
		var findCards = _cards.FindAll(card => card.Rank >= rankMin && card.Rank <= rankMax);
	...
	/// </summary>
	public static Card GetRandomCardWithCondition(CardType type, int rankMin, int rankMax)
	{
		Random random = new Random();
		var findCards = _cards.FindAll(card => card.Type == type && card.Rank >= rankMin && card.Rank <= rankMax);
		return findCards[random.Next(findCards.Count)].Clone();
	}
